...
void ProcessBandOMP(OutType &output, const std::vector<int> &padded_image, int padded_width, int width, int /*height*/,
                    int start_row, int end_row) {
  const int num_bands = 4;
  const int band_width = std::max(width / num_bands, 1);

#pragma omp parallel for default(none) \
    shared(output, padded_image, padded_width, width, start_row, end_row, band_width, num_bands) schedule(static)
...
OutType SolveALL(const std::vector<int> &padded_image, int width, int height, int start_row, int end_row) {
  OutType output;
  output.width = width;
...
void BroadcastImageData(int rank, int &width, int &height, int &total_pixels, std::vector<int> &all_data,
                        const InType &input) {
  if (rank == 0) {
    width = input.width;
    height = input.height;
...
void FillPaddedPixel(std::vector<int> &padded_image, const std::vector<int> &local_data, int width, int height,
                     int local_start, int local_height, int row, int col, int padded_width) {
...
void CreatePaddedImage(const std::vector<int> &local_data, int width, int height, int local_start, int local_height,
                       std::vector<int> &padded_image) {
  ...
  for (int row = 0; row < padded_height; ++row) {
    for (int col = 0; col < padded_width; ++col) {
      FillPaddedPixel(padded_image, local_data, width, height, local_start, local_height, row, col, padded_width);
    }
  }
}
...
void ReceiveProcData(int width, int proc_start, int proc_end, std::vector<int> &output_data) {
  ...
  for (int row = 0; row < proc_rows; ++row) {
    for (int col = 0; col < width; ++col) {
      ...
      output_data[out_idx] =
          proc_data[(static_cast<size_t>(row) * static_cast<size_t>(width)) + static_cast<size_t>(col)];
    }
  }
}
...
void GatherResultsToRoot(int rank, int size, const OutType &local_output, int width, int /*height*/, int start_row,
                         int end_row, int rows_per_proc, int remainder, OutType &output) {
  if (rank == 0) {
    output.width = width;
...
bool TerekhovDGaussVertALL::RunImpl() {
  ...
  std::vector<int> local_data(
      all_data.begin() + static_cast<std::ptrdiff_t>(local_start) * static_cast<std::ptrdiff_t>(width_),
  ...
  GatherResultsToRoot(rank, size, local_output, width_, height_, start_row, end_row, rows_per_proc, remainder, output);
